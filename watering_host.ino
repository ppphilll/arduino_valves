...
void setup(void){
  ...
  for (int i=0; i<sizeof(configuration.valves); i++){
    if (configuration.valves[i] != configuration.dummy_valve){
  ...
  Serial.println("WIFI values in EEPROM");
...
void handleAdminSave(){
  ...
  Serial.println("RAW server args");
  ...
  Serial.println("Received values for configuration");
  ...
  if (strlen(_ssid)!=0 && strlen(_password)!=0 && strlen(_host)!=0){
    //update the ssid and password
  ...
  Serial.println("Object stored values for configuration");
...
void handleRequest(){
  ...
  //valves
  page += "<div id='valves'>";
  for (int i=0; i<configuration.getValveCount(); i++){
...
void handleAjax(){
  ...
  Serial.println("Valve status change");
  ...
  if (valveno!=configuration.dummy_valve){
    digitalWrite(configuration.valves[valveno], (valvestatus==1 ? HIGH : LOW));
  ...
  for (int i=0; i<configuration.getValveCount(); i++){
    bool isDummy = (configuration.valves[i] == configuration.dummy_valve);
    String _state = (configuration.valveStatuses[i]==true ? "ON" : "OFF");
...
String makeValve(int i, bool state, bool isDummy){
  ...
  String _state = (state==true ? "ON" : "OFF");
...
void setupWIFI(){
  ...
  if (strlen(wifi_configuration.ssid)==0 || strlen(wifi_configuration.password)==0){
    setupAP();
    return;
  }
  ...
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print(".");
    lcd.print(".");  //print the dots showing a connection is being made
    waitCount++;
